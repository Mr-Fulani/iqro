...
- [x] Адаптировать scan-Мусхаф к альбомной ориентации без растяжения каллиграфии: страница
  заполняет ширину без пустых боковых полос, доступна по вертикали, горизонтальный свайп
  перелистывает страницы, а hit map аятов и портретная компоновка сохраняются; при возврате
  в портрет альбомное смещение/масштаб сбрасываются и не могут увести страницу за экран.
- [x] Поддержать чёткие scan-страницы для плотных экранов и альбомного чтения: полный
  dataset `madani-hafs@1.0.3` содержит варианты 900/2700 px, а мобильный клиент выбирает
  подходящий размер и обновляет устаревшие метаданные при недостаточном разрешении.
  Обновление assets сохраняет текст, навигацию и 12 346 областей аятов; полный офлайн-пакет
  2700 px занимает около 183 МиБ. Установка APK сама по себе не обновляет старый офлайн-пакет.
- [x] Фильтровать каталог чтецов по виду чтения с исходным «Мурратталь»; по тапу на
  карточку/аватар выбирать вариант текущего фильтра, запускать текущую суру (или Аль-Фатиху)
  и сразу открывать плеер без промежуточной кнопки «Воспроизвести».
- [x] Добавить выбор чтеца и варианта чтения непосредственно в развёрнутом плеере: текущая
  сура и play/pause-состояние сохраняются, а позиция переносится по доле текущего аята только
  при наличии совместимых таймкодов у обеих декламаций.
...
- [x] Довести навигацию Мусхафа до parity с backend: быстрый переход по суре, аяту, странице,
  джузу, хизбу и рубу; поиск не должен обещать неподдерживаемые типы запроса.
- [x] Добавить в читалку пользовательские настройки типографики текста, интервалов и режима
  без отвлечений, не затрагивая точную геометрию отсканированных страниц.
- [x] Провести первый performance-hardening Мусхафа и плеера: частый position tick обновляет
  только transport-панель, статичная часть развёрнутого плеера не перестраивается; параллельные
  запросы одной scan-страницы объединяются, а повторная проверка SHA-256 использует ограниченный
  file-stamp cache с инвалидированием при изменении файла.
- [x] Ограничить глобальную реактивность аудио: position tick больше не перестраивает AppShell
  со всеми вкладками, Home, каталог чтецов и мини-плеер; непрерывная позиция остаётся только в
  transport-панели полного плеера и активной аудиокарточке Du’a.
- [x] Устранить отдельную реализацию audio pill в scan-Мусхафе: AppShell и полноэкранный
  Мусхаф используют один `IqroMiniPlayer` с одинаковой геометрией, портретом, переходами между
  сурами и открытием полного плеера по тапу.
- [x] Провести второй profile/performance проход на Redmi Note 7: контрольный холодный старт
  итогового APK занимает 1,90–1,94 с, hot resume — 109–129 мс; Home/Mushaf/player удерживаются
  примерно в 138/181/168 МБ PSS. У прозрачного общего мини-плеера удалён постоянный GPU blur,
  а быстрые перелистывания Мусхафа больше не создают конкурентный хвост сохранений каждой
  промежуточной страницы — выполняется текущая операция и последняя актуальная.
...
- [x] Добавить понятное управление занятым местом: проверенный manifest показывает размер до
  загрузки, общий лимит приложения равен 3 ГБ, экран настроек перечисляет полные и частичные
  Mushaf/audio-пакеты с фактически занятым местом. Удаление адресное, path-safe и выполняется
  только после подтверждения; активная загрузка защищена, автоматическая очистка запрещена.
...
- [x] Подключить динамический каталог чтецов и декламаций к draft-first backend publication:
  ...
  варианты чтения одного человека сохраняют собственные recitation ID, но используют единый
  канонический портрет в каталоге, полном/мини-плеере, Мусхафе и media metadata.
- [x] Добавить выбор разрешённого bitrate/rendition и завершить проверку покрытия всех
  публикуемых декламаций: Flutter показывает только варианты из API, хранит предпочтение и
  безопасно использует backend default при отсутствии выбранного качества. На staging
  2026-09-04 все 18 опубликованных декламаций заявляют полные 114 сур и 6 236 тайм-сегментов;
  сейчас у внешнего каталога опубликован только `standard` 128 кбит/с, новые `economy/high`
  появятся в UI без выпуска приложения после их публикации в API.
...
- [x] Добавить приватный fallback выбора города с IANA timezone и расчёт направления Киблы
...
- [x] Подключить мобильный План к backend-контрактам: server-authoritative норма в минутах,
  страницах или аятах, ручные reading sessions, 14-дневная история, серии и revision-safe
  prayer check-ins; последний успешный dashboard доступен для безопасного просмотра offline.
- [x] Добавить автоматический зачёт фактического чтения из Flutter reader lifecycle: активное
  время останавливается в фоне, повторные страницы/аяты не дублируются, навигационные прыжки
  не считаются чтением, а idempotent сессия сохраняется и повторно доставляется после offline.
