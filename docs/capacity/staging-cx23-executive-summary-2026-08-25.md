...
### Предварительный порог для 4 vCPU / 8 GiB / 80 GB

Такой сервер ещё не проходил тот же corpus тестов, поэтому следующие числа — плановая
экстраполяция с CX23, а не новый доказанный рекорд. Удвоение CPU обычно даёт здесь около
`1,6–1,8x`, а не ровно `2x`: часть времени занимают PostgreSQL, Redis, gateway и фоновые задачи.
RAM на CX23 не была узким местом, а размер диска почти не влияет на количество одновременных
пользователей.

- **30–40 одновременно активных обычных пользователей** — безопасный стартовый operational
  envelope с запасом для пиков;
- **35–45 активных пользователей** — наиболее вероятная стабильная mixed-граница после запуска
  `2 web + 2 API`, которую всё равно надо подтвердить коротким прогоном на самой машине;
- **80–100 одновременно открытых/периодически активных сессий** вероятно продолжат работать,
  но редкие ответы могут выйти за p99 1 500 ms, поэтому эту цифру нельзя обещать как SLO;
- при модели «одна 10-минутная сессия в день, peak factor 3» диапазон 30–40 peak active
  соответствует примерно **1 400–1 900 DAU**; для простого планирования округляется до
  **1 000–2 000 DAU**;
- 80 GB дают место для образов, журналов и локального backup, но не повышают request capacity;
- аудиоплееры не входят в этот лимит, пока байты идут напрямую через R2/QF CDN. На R2 доказано
  25 параллельных warm-плееров, но это нижняя граница тестового маршрута, а не предел VPS или
  Cloudflare.

До появления production-метрик публично обещать следует именно **30–40 активных одновременно**,
а не верхнюю burst-оценку. Первая неделя заменит эту модель фактическими session duration,
application RPS, CPU и p95/p99.
