...
Web Push доставляет правила `prayer` и `quran_review`. Отдельный ежедневный тип
`quran_reading` сохранён в синхронизационном контракте для обратной совместимости, но новые
Web Push schedules для него не создаются, старые удаляются миграцией и dispatcher их не
захватывает. План чтения после намаза управляет собственным `notifications_enabled`: выключение
убирает текст и deep link чтения из prayer push, не отключая уведомление о самом намазе.
...
Фактический staging пример с синтетическим 8 MiB объектом, успешной ступенью 25 playback
clients и точками деградации 30/35/50 сохранён в
[R2 synthetic audio evidence](capacity/staging-r2-synthetic-audio-2026-08-25.md). Он проверяет
warm CDN/Range и отсутствие нагрузки на VPS, но не заменяет тест разрешённого реального
multi-reciter release.
