...
Статус на 13 сентября 2026 года: по сообщению владельца тестовый сервер временно удалён.
Описания работающей среды ниже являются историческими; они не подтверждают текущую
доступность сервера или работу таймеров. Для будущего production см.
[архитектуру резервного копирования и восстановления](backup-recovery-architecture.md).
...
Для напоминаний о чтении Корана и повторении аятов не нужен платный сервис: браузеры используют
стандартный Web Push, а приложение подписывает сообщения собственной парой VAPID-ключей.
Сгенерировать отдельную пару для staging и безопасно записать её в `staging.env`:

```bash
make staging-web-push-configure
make staging-preflight
```

Команда использует OpenSSL, не печатает закрытый ключ и сохраняет env с правами `0600`. После
этого пересоздайте `backend`, `worker`, `beat` и `web`. В кабинете пользователь должен сам нажать
«Разрешить уведомления» и подтвердить системный запрос браузера. На iPhone Web Push доступен для
сайта, добавленного на главный экран.

Сервер хранит только push-endpoint, открытые ключи подписки, язык и IANA timezone конкретного
устройства. Координаты не сохраняются. Поэтому web доставляет напоминания с фиксированным
локальным временем (`quran_reading`, `quran_review`), а prayer-правила пока только
синхронизируются для будущего мобильного локального scheduler. При переносе на новый VPS с тем
же доменом следует перенести VAPID-ключи как секреты: тогда существующие подписки продолжат
работать. Новая пара ключей потребует повторного разрешения на устройствах.
...
Стандартные backup-команды теперь сохраняют все прежние копии. Offsite upload не
запускает prune; `BACKUP_RETENTION_DAYS` больше не включает локальное удаление.
`--keep-existing` сохранён для совместимости. Новый модуль запускается через
`python -m ops.postgres.offsite`; Compose уже использует этот entrypoint.
Подробности: [backup/recovery runbook](backup-recovery.md).
...
- Web Push настроен отдельной VAPID-парой, реальная подписка и закрытая вкладка проверены;
