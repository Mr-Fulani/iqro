...
class QuranFoundationSyncState(BaseModel):
    """Freshness checkpoint for one cached Quran.Foundation chapter reciter."""

    environment = models.CharField(max_length=16)
    source_reciter_id = models.PositiveIntegerField()
    content_sync_resource_id = models.PositiveIntegerField(null=True, blank=True)
    ...
    def __str__(self) -> str:
        return f"QF {self.environment} chapter-reciter {self.source_reciter_id}"
