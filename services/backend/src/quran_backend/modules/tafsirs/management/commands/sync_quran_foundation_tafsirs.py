...
class Command(BaseCommand):
    help = "Synchronize configured Quran Tafsirs from Quran.Foundation Content Sync."
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        parser.add_argument(
            ...
            type=int,
            dest="resource_ids",
            help="Quran.Foundation Tafsir ID. Repeat to import multiple editions.",
        )
        parser.add_argument(
            "--force",
            action="store_true",
    ...
    def handle(self, *args: Any, **options: Any) -> None:  # noqa: ARG002
        resource_ids = tuple(options["resource_ids"] or settings.QURAN_QF_TAFSIR_RESOURCE_IDS)
        try:
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Tafsirs synchronized: {result.editions} active editions, "
                f"{result.versions_created} new versions, "
                f"{result.records_imported} records imported, "
