...
REQUIRED_FILES = ("surahs.json", "ayahs.jsonl", "pages.jsonl", "juz.json")
...
def validate_quran_dataset(root: Path) -> ValidatedQuranDataset:
    ...
    if manifest.get("schema_version") != DATASET_SCHEMA_VERSION:
        raise QuranDatasetError(f"Unsupported schema_version; expected {DATASET_SCHEMA_VERSION}.")
    ...
    aggregate_checksum = _aggregate_checksum(file_hashes)
...
def _validate_manifest_metadata(manifest: dict[str, Any]) -> None:
    ...
    for field in ("surahs", "ayahs", "pages", "juz"):
...
def _validate_content(
    ...
    _validate_declared_counts(counts, surahs, ayahs, pages, juz)
    ...
    ayah_keys, surah_counts = _collect_ayah_keys(ayahs, int(counts["juz"]))
    _validate_surah_ayah_counts(surahs, ayah_keys, surah_counts)
    _validate_page_mappings(pages, ayah_keys)
    _validate_juz_boundaries(juz, ayah_keys)
...
def _validate_juz_boundaries(
    ...
    for row in juz:
        for boundary in ("start", "end"):
            ...
            if key not in ayah_keys:
                raise QuranDatasetError(f"Juz {row['number']} has an invalid {boundary} ayah.")
...
def _aggregate_checksum(file_hashes: dict[str, Any]) -> str:
