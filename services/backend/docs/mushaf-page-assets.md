...
Полученный каталог ещё не является опубликованным контентом. Загрузка в S3/CDN,
преобразование записей manifest в `pages.jsonl`, привязка интерактивных областей аятов и
публикация версии выполняются отдельным контролируемым процессом.
