...
Для локального Flutter-клиента основной путь — `make dev-data` из корня репозитория:
он создаёт KFGQPC из слов/шрифта и публикует страницы через `/api/v1/quran/mushaf-renditions`
в локальные PostgreSQL и `media/`. См. [инструкцию команды](../../../docs/local-development.md).
Описанный ниже контракт `native_rendering` относится к отдельному управляемому pipeline
публикации источников; объектное хранилище не требуется новому разработчику.
