  ...
  dataRights: {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: ["После периода отмены удаляются либо необратимо обезличиваются аккаунт, email-привязка, устройства и credentials, активные сессии, позиция и сессии чтения, ежедневные нормы, прогресс, серии дней, закладки, напоминания, персональные настройки и связанные обращения, кроме данных, которые необходимо временно сохранить по закону, для безопасности или разрешения спора.", "Рабочая копия удаляется не позднее 30 дней после подтверждённого запроса. Остаточные копии исчезают из ротационных резервных копий не позднее 90 дней и до этого не возвращаются в рабочую систему. Минимальные security-журналы могут храниться ограниченный срок с ограниченным доступом."],
      },
      {
...
const en = (locale: Locale, config: LegalConfig): AdditionalLegalCopy => ({
  ...
  dataRights: {
    ...
    sections: [
      ...
      { title: "4. What is deleted and when", paragraphs: ["After the cancellation period, we delete or irreversibly de-identify the account, email link, devices and credentials, active sessions, reading position and sessions, daily goals, progress, streaks, bookmarks, reminders, personal settings, and related support records, except data temporarily required by law, security, or a dispute.", "The working copy is erased no later than 30 days after the confirmed request. Residual copies age out of rotating backups within 90 days and are not restored to production. Minimal security logs may remain for a limited period under restricted access."] },
