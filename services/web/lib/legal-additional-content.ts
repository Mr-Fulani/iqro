  ...
  dataRights: {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: ["После периода отмены удаляются либо необратимо обезличиваются аккаунт, email-привязка, устройства и credentials, активные сессии, позиция и сессии чтения, ежедневные нормы, планы и отметки чтения после намазов, прогресс, серии дней, закладки, напоминания, персональные настройки и связанные обращения, кроме данных, которые необходимо временно сохранить по закону, для безопасности или разрешения спора.", "Рабочая копия удаляется не позднее 30 дней после подтверждённого запроса. Остаточные копии исчезают из ротационных резервных копий не позднее 90 дней и до этого не возвращаются в рабочую систему. Минимальные security-журналы могут храниться ограниченный срок с ограниченным доступом."],
      },
      {
...
const en = (locale: Locale, config: LegalConfig): AdditionalLegalCopy => ({
  ...
  dataRights: {
    ...
    sections: [
      ...
      { title: "4. What is deleted and when", paragraphs: ["After the cancellation period, we delete or irreversibly de-identify the account, email link, devices and credentials, active sessions, reading position and sessions, daily goals, after-prayer reading plans and check-ins, progress, streaks, bookmarks, reminders, personal settings, and related support records, except data temporarily required by law, security, or a dispute.", "The working copy is erased no later than 30 days after the confirmed request. Residual copies age out of rotating backups within 90 days and are not restored to production. Minimal security logs may remain for a limited period under restricted access."] },
...
const ar = (locale: Locale, config: LegalConfig): AdditionalLegalCopy => ({
  ...
  cookies: {
    ...
    sections: [
      ...
      { title: "3. التخزين المحلي والمؤقت", paragraphs: ["يحفظ localStorage مؤشر المزامنة وطابور التغييرات غير المرسلة وطابورًا محدودًا لجلسات القراءة غير المرسلة والحالة التقنية وآخر موقع وطريقة حساب ومذهب عصر اختيرت على هذا الجهاز. ويحفظ sessionStorage مؤقتًا مسار الرجوع الآمن وإشعارات دورة الحساب. تبقى البيانات على الجهاز حتى يمسحها المتصفح أو التطبيق."] },
  ...
  dataRights: {
    ...
    sections: [
      ...
      { title: "4. ما يُحذف ومتى", paragraphs: ["بعد مهلة الإلغاء نحذف الحساب وربط البريد والأجهزة وبيانات الاعتماد والجلسات وموضع القراءة وجلساتها والأهداف اليومية والتقدم وسلاسل الأيام والعلامات والتذكيرات والإعدادات وسجلات الدعم أو نزيل ارتباطها بالشخص، باستثناء ما يلزم مؤقتًا للقانون أو الأمن أو النزاع.", "تُحذف النسخة العاملة خلال 30 يومًا من الطلب المؤكد، وتختفي البقايا من النسخ الاحتياطية الدورية خلال 90 يومًا ولا تعاد إلى الإنتاج. قد تبقى سجلات أمنية محدودة بوصول مقيد."] },
...
const tr = (locale: Locale, config: LegalConfig): AdditionalLegalCopy => ({
  ...
  dataRights: {
    ...
    sections: [
      ...
      { title: "4. Ne zaman ne silinir", paragraphs: ["İptal süresinden sonra hesap, e-posta bağlantısı, cihaz ve credentials, oturumlar, okuma konumu ve oturumları, günlük hedefler, ilerleme, seriler, yer imleri, hatırlatıcılar, ayarlar ve ilgili destek kayıtları silinir veya geri döndürülemez biçimde kimliksizleştirilir; yasa, güvenlik veya uyuşmazlık için geçici olarak gerekenler hariçtir.", "Çalışan kopya doğrulanmış talepten sonra en geç 30 günde silinir. Kalıntılar dönen yedeklerden 90 gün içinde çıkar ve production'a geri yüklenmez. Asgari güvenlik kayıtları sınırlı süre ve kısıtlı erişimle kalabilir."] },
