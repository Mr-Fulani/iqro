...
const ru = (locale: Locale, config: LegalConfig): AdditionalLegalCopy => ({
  ...
  providers: {
    ...
    sections: [
      ...
      {
        title: "4. Email и мониторинг",
        paragraphs: ["Production-поставщики доставки email и внешнего мониторинга ещё не выбраны. Функции, которым они нужны, не должны включаться в публичном production до выбора поставщика, заключения необходимых соглашений и добавления его названия, цели, страны обработки и прямой ссылки на политику на эту страницу."],
      },
      {
...
const en = (locale: Locale, config: LegalConfig): AdditionalLegalCopy => ({
  ...
  providers: {
    ...
    sections: [
      ...
      { title: "4. Email and monitoring", paragraphs: ["Production email-delivery and external-monitoring providers have not been selected. Features needing them must not be enabled in public production until a provider is selected, required agreements are in place, and its name, purpose, processing country, and direct privacy link are published here."] },
...
const ar = (locale: Locale, config: LegalConfig): AdditionalLegalCopy => ({
  ...
  cookies: {
    ...
    sections: [
      ...
      { title: "3. التخزين المحلي والمؤقت", paragraphs: ["يحفظ localStorage مؤشر المزامنة وطابور التغييرات غير المرسلة والحالة التقنية. ويحفظ sessionStorage مؤقتًا مسار الرجوع الآمن وإشعارات دورة الحساب. تبقى البيانات على الجهاز حتى يمسحها المتصفح أو التطبيق."] },
  ...
  providers: {
    ...
    sections: [
      ...
      { title: "4. البريد والمراقبة", paragraphs: ["لم يُختر بعد مزود production للبريد أو المراقبة الخارجية. لا تُفعّل الوظائف المعتمدة عليهما للعامة قبل الاختيار والاتفاقات اللازمة ونشر الاسم والغرض ودولة المعالجة ورابط الخصوصية هنا."] },
...
const tr = (locale: Locale, config: LegalConfig): AdditionalLegalCopy => ({
  ...
  cookies: {
    ...
    sections: [
      ...
      { title: "3. localStorage ve sessionStorage", paragraphs: ["localStorage yerel cursor'ı, gönderilmemiş değişiklik kuyruğunu ve eşitleme durumunu tutar. sessionStorage güvenli giriş dönüş yolunu ve tek kullanımlık hesap bildirimlerini geçici tutar. Tarayıcı veya uygulama temizleyene kadar cihazda kalır."] },
  ...
  providers: {
    ...
    sections: [
      ...
      { title: "4. E-posta ve izleme", paragraphs: ["Production e-posta teslimi ve harici izleme sağlayıcıları henüz seçilmedi. Bunlara bağlı özellikler; sağlayıcı seçilmeden, sözleşmeler kurulmadan ve adı, amacı, işleme ülkesi ile doğrudan gizlilik bağlantısı burada yayımlanmadan halka açılmamalıdır."] },
