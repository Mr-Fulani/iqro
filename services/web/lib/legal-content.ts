...
const ru = (config: LegalConfig): LegalCopy => ({
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          "позиция чтения, ежедневная норма, план и отметки чтения после намазов, сессии чтения, прогресс и серия выполненных дней, закладки, напоминания, настройки намаза и синхронизации; при включении уведомлений — браузерный push-endpoint и открытые ключи подписки; после отдельного разрешения геолокации — координаты устройства только для расчёта намазных уведомлений;",
      ...
      },
      {
        ...
        paragraphs: [
          "Необходимые HttpOnly cookies хранят refresh-сессию и ключ установки; cookie языка хранит выбранную локаль. В localStorage сохраняются очередь синхронизации, ограниченная очередь неотправленных сессий чтения, служебное состояние, последнее выбранное на этом устройстве место, метод расчёта и мазхаб Асра, а sessionStorage — временный маршрут возврата и уведомления. Эти механизмы нужны для запрошенной функциональности, а не для рекламного профилирования.",
        ],
      },
      {
  ...
  sources: {
    title: "Источники и лицензии", description: "Происхождение, версии, правообладатели и условия использования опубликованного Quran, аудио- и ду’а-контента.", quranTitle: "Издания Корана", audioTitle: "Аудиодекламации", duaTitle: "Ду’а и азкары", duaBody: "Полный каталог из 267 карточек и 132 тем основан на языковых изданиях «Хисн аль-Муслим» от IslamHouse. Текст и смысл сохраняются с языковой версией, идентификатором источника и версией снимка.", duaRights: "Условия IslamHouse разрешают распространение материалов без изменения научного содержания при указании источника. Перед публичным production-запуском команда дополнительно запрашивает письменное подтверждение карточного формата.", duaVerification: "Ссылки на хадисы сейчас помечаются как перенесённые из издания. Статус отдельной редакционной проверки появляется только после фактической сверки; автоматическое сопоставление с HadeethEnc не считается проверкой.", version: "Версия", source: "Источник", license: "Лицензия", rightsHolder: "Правообладатель", attribution: "Атрибуция", noQuran: "Нет опубликованных изданий.", noAudio: "Нет опубликованных лицензированных декламаций.", noticeTitle: "Важно", noticeBody: "Публикуются только активные versioned материалы с зафиксированным источником и лицензией. Условия относятся к контенту, а не автоматически ко всему программному продукту. Прямые media URL намеренно не выводятся на этой странице.",
  },
});
  ...
  return {
    privacy: {
      ...
      sections: [
        ...
        { title: "2. Data we process", paragraphs: ["The data depends on the features you use."], bullets: ["installation and device identifiers, app version, language, time zone, sessions, and security events;", "email and verification state for one-time-code sign-in;", "reading position, daily goal, after-prayer reading plan and check-ins, reading sessions, progress and reading-day streak, bookmarks, reminders, prayer settings, and sync state; when notifications are enabled, the browser push endpoint and public subscription keys; after separate location permission, device coordinates used only to calculate prayer notifications;", "support ticket subject, messages, and history;", "IP address, request ID, and minimal request logs for security, diagnostics, and abuse prevention."] },
        ...
        { title: "4. Cookies and local storage", paragraphs: ["Necessary HttpOnly cookies hold the refresh session and installation key; a locale cookie remembers language. localStorage holds the sync queue, a bounded queue of unsent reading sessions, operational state, and the last prayer-calculation location, method, and Asr school selected on this device, while sessionStorage holds temporary return paths and notices. These mechanisms support requested functionality and are not used for advertising profiles."] },
    ...
    sources: { title: "Sources and licenses", description: "Provenance, versions, rights holders, and usage terms for published Quran, audio, and dua content.", quranTitle: "Quran editions", audioTitle: "Audio recitations", duaTitle: "Duas and adhkar", duaBody: "The complete catalog of 267 cards and 132 topics is based on the IslamHouse language editions of Hisn al-Muslim. Text and meaning retain their language edition, source identifier, and snapshot version.", duaRights: "IslamHouse terms permit distribution without changing the scholarly content and with source attribution. The team is also seeking written confirmation for the card format before the public production launch.", duaVerification: "Hadith references are currently marked as copied from the source edition. Independent editorial verification is shown only after it actually occurs; automated HadeethEnc matching does not count as verification.", version: "Version", source: "Source", license: "License", rightsHolder: "Rights holder", attribution: "Attribution", noQuran: "No published editions.", noAudio: "No published licensed recitations.", noticeTitle: "Important", noticeBody: "Only active versioned material with recorded provenance and licensing is published. Content terms do not automatically apply to the entire software product. Direct media URLs are intentionally omitted from this page." },
  };
};
  ...
  return {
    ...
    privacy: {
      ...
      sections: [
        ...
        { title: "2. البيانات التي نعالجها", paragraphs: ["تختلف البيانات بحسب الوظائف التي تستخدمها."], bullets: ["معرّفات التثبيت والجهاز وإصدار التطبيق واللغة والمنطقة الزمنية والجلسات وأحداث الأمن؛", "البريد الإلكتروني وحالة التحقق عند الدخول بالرمز المؤقت؛", "موضع القراءة والهدف اليومي وجلسات القراءة والتقدم وسلسلة الأيام المكتملة والعلامات والتذكيرات وإعدادات الصلاة وحالة المزامنة؛ وعند تفعيل الإشعارات عنوان push للمتصفح ومفاتيح الاشتراك العامة؛ وبعد إذن منفصل للموقع إحداثيات الجهاز المستخدمة فقط لحساب إشعارات الصلاة؛", "موضوع رسائل الدعم ومحتواها وسجلها؛", "عنوان IP ومعرّف الطلب وسجلات تقنية محدودة للأمن والتشخيص ومنع الإساءة."] },
        ...
        { title: "4. ملفات الارتباط والتخزين المحلي", paragraphs: ["تحفظ ملفات HttpOnly الضرورية جلسة التحديث ومفتاح التثبيت، ويحفظ ملف اللغة اختيارك. يحتفظ localStorage بطابور المزامنة وطابور محدود لجلسات القراءة غير المرسلة والحالة التشغيلية وآخر موقع وطريقة حساب ومذهب عصر اختيرت على هذا الجهاز، ويحتفظ sessionStorage بمسار الرجوع والإشعارات المؤقتة. تُستخدم هذه الوسائل للوظائف المطلوبة لا لبناء ملفات إعلانية."] },
    ...
    sources: { ...copy.sources, title: "المصادر والتراخيص", description: "مصدر وإصدار وحقوق وشروط استخدام محتوى القرآن والصوت والدعاء المنشور.", quranTitle: "طبعات القرآن", audioTitle: "التلاوات الصوتية", duaTitle: "الأدعية والأذكار", duaBody: "يعتمد الدليل الكامل، الذي يضم ٢٦٧ بطاقة و١٣٢ موضوعًا، على طبعات «حصن المسلم» اللغوية المنشورة في IslamHouse، مع حفظ لغة الطبعة ومعرّف المصدر وإصدار اللقطة.", duaRights: "تسمح شروط IslamHouse بنشر المواد من دون تغيير المحتوى العلمي مع ذكر المصدر. ويطلب الفريق أيضًا تأكيدًا كتابيًا لصيغة البطاقات قبل الإطلاق العام.", duaVerification: "تُوسم إحالات الحديث حاليًا بأنها منقولة من طبعة المصدر. ولا يظهر توثيق المراجعة التحريرية المستقلة إلا بعد إجرائها فعلًا؛ ولا يُعد الربط الآلي مع HadeethEnc مراجعة.", version: "الإصدار", source: "المصدر", license: "الترخيص", rightsHolder: "صاحب الحقوق", attribution: "نَسب المصدر", noQuran: "لا توجد طبعات منشورة.", noAudio: "لا توجد تلاوات مرخصة منشورة.", noticeTitle: "مهم", noticeBody: "لا يُنشر إلا المحتوى النشط ذو الإصدار والمصدر والترخيص المسجلين. لا تسري شروط المحتوى تلقائيًا على المنتج البرمجي كله. لا تُعرض روابط الوسائط المباشرة هنا." },
  };
};
  ...
  return {
    ...
    privacy: {
      ...
      sections: [
        ...
        { title: "2. İşlediğimiz veriler", paragraphs: ["Veri kapsamı kullandığınız özelliklere göre değişir."], bullets: ["kurulum ve cihaz tanımlayıcıları, uygulama sürümü, dil, saat dilimi, oturumlar ve güvenlik olayları;", "tek kullanımlık kodla giriş için e-posta ve doğrulama durumu;", "okuma konumu, günlük hedef, okuma oturumları, ilerleme ve tamamlanan gün serisi, yer imleri, hatırlatıcılar, namaz ayarları ve senkronizasyon durumu; bildirimler açıldığında tarayıcı push endpoint'i ve açık abonelik anahtarları; ayrı konum izninden sonra yalnızca namaz bildirimlerini hesaplamak için kullanılan cihaz koordinatları;", "destek talebinin konusu, mesajları ve geçmişi;", "güvenlik, teşhis ve kötüye kullanımı önleme için IP adresi, istek kimliği ve sınırlı teknik kayıtlar."] },
        ...
        { title: "4. Çerezler ve yerel depolama", paragraphs: ["Zorunlu HttpOnly çerezleri yenileme oturumunu ve kurulum anahtarını, dil çerezi ise seçiminizi saklar. localStorage senkronizasyon kuyruğunu, gönderilmemiş okuma oturumları için sınırlı bir kuyruğu, işletim durumunu ve bu cihazda seçilen son namaz hesaplama konumunu, yöntemini ve ikindi mezhebini; sessionStorage geçici dönüş yolunu ve bildirimleri tutar. Bunlar istenen işlevler içindir, reklam profili oluşturmak için kullanılmaz."] },
    ...
    sources: { ...copy.sources, title: "Kaynaklar ve lisanslar", description: "Yayımlanmış Kur'an, ses ve dua içeriğinin kaynağı, sürümü, hak sahipleri ve kullanım koşulları.", quranTitle: "Kur'an baskıları", audioTitle: "Sesli kıraatler", duaTitle: "Dua ve zikirler", duaBody: "267 kart ve 132 konudan oluşan tam katalog IslamHouse'taki Hısnu'l-Müslim dil baskılarına dayanır; baskı dili, kaynak kimliği ve anlık görüntü sürümü korunur.", duaRights: "IslamHouse koşulları, ilmî içerik değiştirilmeden ve kaynak gösterilerek dağıtıma izin verir. Ekip, herkese açık production yayını öncesinde kart biçimi için ayrıca yazılı onay istemektedir.", duaVerification: "Hadis referansları şu anda kaynak baskıdan aktarılmış olarak işaretlenir. Bağımsız editoryal doğrulama yalnızca gerçekten yapıldıktan sonra gösterilir; otomatik HadeethEnc eşleştirmesi doğrulama sayılmaz.", version: "Sürüm", source: "Kaynak", license: "Lisans", rightsHolder: "Hak sahibi", attribution: "Atıf", noQuran: "Yayımlanmış baskı yok.", noAudio: "Yayımlanmış lisanslı kıraat yok.", noticeTitle: "Önemli", noticeBody: "Yalnızca kaynağı ve lisansı kayıtlı etkin sürümlü içerik yayımlanır. İçerik koşulları tüm yazılım ürünü için otomatik olarak geçerli değildir. Doğrudan medya URL'leri bu sayfada özellikle gösterilmez." },
  };
};
