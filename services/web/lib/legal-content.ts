    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          "позиция чтения, закладки, напоминания, настройки намаза и синхронизации; при включении уведомлений — браузерный push-endpoint и открытые ключи подписки; после отдельного разрешения геолокации — координаты устройства только для расчёта намазных уведомлений;",
  ...
  return {
    privacy: {
      ...
      sections: [
        ...
        { title: "2. Data we process", paragraphs: ["The data depends on the features you use."], bullets: ["installation and device identifiers, app version, language, time zone, sessions, and security events;", "email and verification state for one-time-code sign-in;", "reading position, bookmarks, reminders, prayer settings, and sync state; when notifications are enabled, the browser push endpoint and public subscription keys; after separate location permission, device coordinates used only to calculate prayer notifications;", "support ticket subject, messages, and history;", "IP address, request ID, and minimal request logs for security, diagnostics, and abuse prevention."] },
        ...
        { title: "4. Cookies and local storage", paragraphs: ["Necessary HttpOnly cookies hold the refresh session and installation key; a locale cookie remembers language. localStorage holds the sync queue, operational state, and the last prayer-calculation location, method, and Asr school selected on this device, while sessionStorage holds temporary return paths and notices. These mechanisms support requested functionality and are not used for advertising profiles."] },
  ...
  return {
    ...
    privacy: {
      ...
      sections: [
        ...
        { title: "2. البيانات التي نعالجها", paragraphs: ["تختلف البيانات بحسب الوظائف التي تستخدمها."], bullets: ["معرّفات التثبيت والجهاز وإصدار التطبيق واللغة والمنطقة الزمنية والجلسات وأحداث الأمن؛", "البريد الإلكتروني وحالة التحقق عند الدخول بالرمز المؤقت؛", "موضع القراءة والعلامات والتذكيرات وإعدادات الصلاة وحالة المزامنة؛ وعند تفعيل الإشعارات عنوان push للمتصفح ومفاتيح الاشتراك العامة؛ وبعد إذن منفصل للموقع إحداثيات الجهاز المستخدمة فقط لحساب إشعارات الصلاة؛", "موضوع رسائل الدعم ومحتواها وسجلها؛", "عنوان IP ومعرّف الطلب وسجلات تقنية محدودة للأمن والتشخيص ومنع الإساءة."] },
  ...
  return {
    ...
    privacy: {
      ...
      sections: [
        ...
        { title: "2. İşlediğimiz veriler", paragraphs: ["Veri kapsamı kullandığınız özelliklere göre değişir."], bullets: ["kurulum ve cihaz tanımlayıcıları, uygulama sürümü, dil, saat dilimi, oturumlar ve güvenlik olayları;", "tek kullanımlık kodla giriş için e-posta ve doğrulama durumu;", "okuma konumu, yer imleri, hatırlatıcılar, namaz ayarları ve senkronizasyon durumu; bildirimler açıldığında tarayıcı push endpoint'i ve açık abonelik anahtarları; ayrı konum izninden sonra yalnızca namaz bildirimlerini hesaplamak için kullanılan cihaz koordinatları;", "destek talebinin konusu, mesajları ve geçmişi;", "güvenlik, teşhis ve kötüye kullanımı önleme için IP adresi, istek kimliği ve sınırlı teknik kayıtlar."] },
        ...
        { title: "4. Çerezler ve yerel depolama", paragraphs: ["Zorunlu HttpOnly çerezleri yenileme oturumunu ve kurulum anahtarını, dil çerezi ise seçiminizi saklar. localStorage senkronizasyon kuyruğunu, işletim durumunu ve bu cihazda seçilen son namaz hesaplama konumunu, yöntemini ve ikindi mezhebini; sessionStorage geçici dönüş yolunu ve bildirimleri tutar. Bunlar istenen işlevler içindir, reklam profili oluşturmak için kullanılmaz."] },
