  ...
  privacy: {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          "Необходимые HttpOnly cookies хранят refresh-сессию и ключ установки; cookie языка хранит выбранную локаль. В localStorage сохраняются очередь синхронизации, служебное состояние и последнее выбранное на этом устройстве место расчёта намаза, а sessionStorage — временный маршрут возврата и уведомления. Эти механизмы нужны для запрошенной функциональности, а не для рекламного профилирования.",
        ],
      },
      {
  ...
  return {
    privacy: {
      ...
      sections: [
        ...
        { title: "4. Cookies and local storage", paragraphs: ["Necessary HttpOnly cookies hold the refresh session and installation key; a locale cookie remembers language. localStorage holds the sync queue, operational state, and the last prayer-calculation location selected on this device, while sessionStorage holds temporary return paths and notices. These mechanisms support requested functionality and are not used for advertising profiles."] },
