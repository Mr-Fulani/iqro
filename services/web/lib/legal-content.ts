    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          "позиция чтения, закладки, напоминания, настройки намаза и синхронизации; при включении уведомлений — браузерный push-endpoint и открытые ключи подписки;",
  ...
  return {
    privacy: {
      ...
      sections: [
        ...
        { title: "2. Data we process", paragraphs: ["The data depends on the features you use."], bullets: ["installation and device identifiers, app version, language, time zone, sessions, and security events;", "email and verification state for one-time-code sign-in;", "reading position, bookmarks, reminders, prayer settings, and sync state; when notifications are enabled, the browser push endpoint and public subscription keys;", "support ticket subject, messages, and history;", "IP address, request ID, and minimal request logs for security, diagnostics, and abuse prevention."] },
  ...
  return {
    ...
    privacy: {
      ...
      sections: [
        ...
        { title: "2. البيانات التي نعالجها", paragraphs: ["تختلف البيانات بحسب الوظائف التي تستخدمها."], bullets: ["معرّفات التثبيت والجهاز وإصدار التطبيق واللغة والمنطقة الزمنية والجلسات وأحداث الأمن؛", "البريد الإلكتروني وحالة التحقق عند الدخول بالرمز المؤقت؛", "موضع القراءة والعلامات والتذكيرات وإعدادات الصلاة وحالة المزامنة؛ وعند تفعيل الإشعارات عنوان push للمتصفح ومفاتيح الاشتراك العامة؛", "موضوع رسائل الدعم ومحتواها وسجلها؛", "عنوان IP ومعرّف الطلب وسجلات تقنية محدودة للأمن والتشخيص ومنع الإساءة."] },
