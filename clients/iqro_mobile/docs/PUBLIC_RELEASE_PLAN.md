...
### 2026-09-08 — исправление типографики: APK установлен, новые страницы готовятся
...
- Первоначальный запуск тестов остановился из-за **errno 28**. После отдельного
  ...
  **361 passed, 13 opt-in skipped**; `flutter analyze --no-pub` без замечаний.
  Profile APK arm64/staging собран и установлен через `adb install -r` на Redmi
  Note 7 (`91aedea7`), без сброса данных. APK SHA256:
  `c5110f5ce353389509f51645559675834345140e0ffa76c62e3e1ad52b7aeea9`.
  Сохранено пользовательское чтение **QCF, страница 2 / 2:1**. Device QA в работе.
...
- На закреплённых источниках **35 Python tests passed без skips**, включая
  ...
  **4 Node raster tests passed**. Полный source audit 604×2 прошёл: 34 tests,
  ...
  Собираются **новые immutable версии** `qcf-v2-iqro-20260908-v4` и
...
- Проверки Android геометрии: QCF страница 3, короткий тап выделяет **2:7**,
  ...
  Новые изображения ещё не участвовали в этой проверке — они готовятся отдельно.
- Следом: завершить полные bundles; backend/Flutter проверки всех 1812 файлов
  каждого издания; commit; staging content publication и проверка обновления
  offline-пакетов. **Новые страницы ещё не опубликованы**. Backend runtime остаётся
  `5b4ae94`: его код/миграции для этой корректировки не меняются.
