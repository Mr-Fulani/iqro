...
- `AudioController` владеет единственным player instance. Экран аудио одним запросом получает
  ...
  `AudioTrack` сохраняет весь опубликованный backend-список renditions. Предпочтение
  `auto/economy/standard/high` применяется и к свежему ответу, и к кэшу; отсутствующее качество
  fail-safe возвращается к default. Смена физического потока не меняет логический track/timeline
  и восстанавливает позицию, диапазон аятов и play/pause.
...
Размер арабского текста, межстрочный/межаятный интервалы и focus mode принадлежат только
адаптивной текстовой читалке и сохраняются в device preferences. Точная scan-геометрия,
масштаб страницы и hit maps от этих настроек не зависят.
...
`OfflineStorageRepository` строит единый инвентарь Mushaf/audio-пакетов из SQLite и считает
фактический размер только внутри выделенного `ApplicationSupport/offline_packages`. Перед
скачиванием manifest показывает полный размер и проверяется лимит приложения 3 ГБ. Удаление
всегда адресное и подтверждаемое: пути валидируются относительно каталога конкретного пакета,
активная загрузка не удаляется, неожиданные вложенные каталоги приводят к fail-closed отказу,
а метаданные стираются только после успешного удаления файлов. Автоочистки нет.
