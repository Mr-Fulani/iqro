...
Аль-Фатиху), запускает воспроизведение и сразу показывает полный плеер. Несколько
стилей одного чтеца объединяются в одну карточку и выбираются отдельным селектором;
дубли аватаров для одного человека не создаются.
