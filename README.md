...
Если Android-устройство подключено по USB и доступно через ADB, `make up` также
автоматически выполняет `adb reverse tcp:8000 tcp:8000`. Без ADB или подключённого
устройства запуск web/backend продолжается как обычно.
...
Структура строк QUL для всех 13 вариантов уже включена в репозиторий и применяется автоматически:
[правила отображения, источники и обновление пакетов](services/backend/docs/qul-page-layout.md).
